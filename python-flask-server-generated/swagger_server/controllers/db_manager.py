...
class DataBaseManagemantSystem:
    ...
    def update_competition(self, username, competition):
        cur = self.con.cursor()
        request = f"SELECT special_id FROM sport_betting.competitions_info WHERE created_by = '{username}' AND id='{competition['competition_id']}'"
        cur.execute(request)
        ans = cur.fetchone()
        if ans is None:
            raise Exception()
        ...
        cur = self.con.cursor()
        for match in competition['ended_matches']:
            request = f"UPDATE sport_betting.matches_info SET is_active={match['is_active']}, start_time='{match['start_time']}', end_time='{match['end_time']}', first_team_result={match['team1_res']}, second_team_result={match['team2_res']}) WHERE competition_id='{ans[0]}'"
            cur.execute(request)
        cur = self.con.cursor()
        for match in competition['not_ended_matches']:
            ...
            cur.execute(request)
        cur = self.con.cursor()
        request = f"INSERT INTO sport_betting.competition_bets (competition_id, user_id, result) VALUES ('{ans[0]}', '{username}', NULL)"
        cur.execute(request)
        self.con.commit()
