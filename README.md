...
### Делание ставок
Под собой скрывает дергания ручки /create/bet на странице с матчем, ручка проверяет что матч еще не завершен, проверяет что записи по этому матч у данного пользователя еще нет (таблица по матчам и ставкам на матч). Если все сходится, делает запись в таблицу sport_betting.match_bets.
